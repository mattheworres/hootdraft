class CommishPickEntryController {
  ...
  $onChanges(changes) {
    ...
    if (currentPick && currentPick.isFirstChange()) {
      this.currentPick = this.lodash.merge(
        {},
        this.currentPick,
        currentPick.currentValue
      );
    }
    ...
    if (currentPick && currentPick.isFirstChange() === false) {
      ...
      this.isFirstPick =
        (angular.isDefined(localCurrentPick)
          ? localCurrentPick.player_pick
          : 0) === '1';
      ...
      if (this.manualEntry) {
        ...
        this.currentPick.selected = this.pickService.determinePickSelected(
          localCurrentPick
        );
      }
    }
  ...
  manualEntryChange() {
    if (this.manualEntry) {
      //If we have a first AND last name, go ahead and show this as a pick. May not have position coloring, but thats OK.
      const hasFirst =
        angular.isDefined(this.currentPick.first_name) &&
        this.currentPick.first_name.length > 0;
      const hasLast =
        angular.isDefined(this.currentPick.last_name) &&
        this.currentPick.last_name.length > 0;
      const hasTeam =
        angular.isDefined(this.currentPick.team) &&
        this.currentPick.team.length > 0;
      const hasPosition =
        angular.isDefined(this.currentPick.position) &&
        this.currentPick.position.length > 0;
  ...
  _add() {
    ...
    if (this.manualEntry && !this._pickIsTheSame()) {
      const duplicateCheckSuccess = response => {
        if (response.pickIsNotDuplicate === false) {
          ...
          this.messageService.showInfo(
            'Pick was reset - go ahead and enter another player.'
          );
      ...
      const duplicateCheckFailure = response => {
        ...
        if (response.pickIsNotDuplicate) {
          this.messageService.showError(
            'Unable to enter pick - error while checking for duplicates.'
          );
          return;
        }
      ...
      const duplicateResult = this.pickService.checkForExistingPicks(
        this.$routeParams.draft_id,
        this.currentPick
      );

      duplicateResult.promise.then(
  ...
  proPlayerSearch(searchTerm) {
    return this.api.Pick.searchProPlayers({
      league: this.draft.draft_sport,
      searchTerm,
    })
      .$promise.then(data => this.limitToFilter(data.players, 10))
      .catch(() => {
  ...
  selectPlayer(item) {
    ...
    if (!this._pickIsTheSame()) {
      ...
      const duplicateCheckFailure = response => {
        if (response.pickIsNotDuplicate) {
          this.messageService.showError(
            'Unable to select pick - error while checking for duplicates.'
          );
          return;
        }
  ...
  _pickIsTheSame() {
    const firstIsSame =
      this.currentPick.first_name === this.pristinePick.first_name;
    const lastIsSame =
      this.currentPick.last_name === this.pristinePick.last_name;
    const teamIsSame = this.currentPick.team === this.pristinePick.team;
    const positionIsSame =
      this.currentPick.position === this.pristinePick.position;
